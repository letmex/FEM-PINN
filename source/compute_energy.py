...
def field_grads(inp, field, area_elem, T = None):
    if T == None:
        ...
        # IMPORTANT:
        # Use signed element Jacobian (2A_signed) computed from connectivity,
        # instead of externally provided (typically absolute) area.
        # If mesh element orientation is mixed (CW/CCW), dividing by |A| causes
        # spurious sign flips in FE gradients that show up as stripe/radiating artifacts.
        x1 = inp[T[:, 0], -2]
        y1 = inp[T[:, 0], -1]
        x2 = inp[T[:, 1], -2]
        y2 = inp[T[:, 1], -1]
        ...
        twoA_signed = (x2 - x1) * (y3 - y1) - (x3 - x1) * (y2 - y1)
        twoA_safe = torch.where(
            torch.abs(twoA_signed) > torch.finfo(twoA_signed.dtype).tiny,
            twoA_signed,
            torch.sign(twoA_signed) * torch.finfo(twoA_signed.dtype).tiny + (twoA_signed == 0).to(twoA_signed.dtype) * torch.finfo(twoA_signed.dtype).tiny,
        )

        grad_x = (y2 - y3) * field[T[:, 0]] + \
                (y3 - y1) * field[T[:, 1]] + \
                (y1 - y2) * field[T[:, 2]]
        grad_y = (x3 - x2) * field[T[:, 0]] + \
                (x1 - x3) * field[T[:, 1]] + \
                (x2 - x1) * field[T[:, 2]]
        grad_x = grad_x / twoA_safe
        grad_y = grad_y / twoA_safe
