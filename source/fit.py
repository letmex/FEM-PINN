...
from optim import get_optimizer_tm
...
def fit_tm(loss_fn, params, optimizer_dict, n_epochs_lbfgs=0, n_epochs_rprop=0, min_delta=1e-6, writer=None, writer_tag=None):
    """
    Generic optimizer loop for thermo-mechanical and phase-field substeps.
    loss_fn must return tuple(loss, loss_info_dict).
    """
    params = list(params)
    if len(params) == 0:
        return []
    if isinstance(params[0], dict):
        raise ValueError(
            "fit_tm no longer accepts branch parameter-group dicts. "
            "Pass a flat parameter iterable (e.g., model.parameters())."
        )
    ...
    if n_epochs_lbfgs > 0:
        ...
        for epoch in range(n_epochs_lbfgs):
            ...
            loss_lbfgs = optimizer.step(closure=closure)
            loss_lbfgs_val = loss_lbfgs.item()
            if not np.isfinite(loss_lbfgs_val):
                break
            loss_data.append(loss_lbfgs_val)
            if writer is not None and writer_tag is not None:
                writer.add_scalar(writer_tag, loss_lbfgs_val, len(loss_data))
    ...
    if n_epochs_rprop > 0:
        ...
        early_stopping = EarlyStopping(tol_steps=10, min_delta=min_delta, device=device)
        loss_prev = torch.tensor([0.0], device=device)

        for epoch in range(n_epochs_rprop):
            optimizer.zero_grad()
            loss, _ = loss_fn()
            ...
            loss_data.append(loss.item())
            if writer is not None and writer_tag is not None:
                writer.add_scalar(writer_tag, loss.item(), len(loss_data))
